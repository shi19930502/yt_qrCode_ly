...
function renderXsjlList(data, outquarantineinfo) {
	...
	for (var m = 0; m < data.length; m++) {
		...
		if (m >= 1) {
			for (var n = 1; n < 2; n++) {
				htmlStr_left = '<li class="picihao">' + '<span class="yellow_spot ani" swiper-animate-effect="bounceInDown" swiper-animate-duration="1s" swiper-animate-delay=' + animateC + 's></span>' + '<div class="text ani" swiper-animate-effect="bounceInLeft" swiper-animate-duration="1s" swiper-animate-delay=' + animateT + 's>' + '<p>批次号</p>' + '<p class="span">' + data[m][arr[m]].batchId + '</p>' + '<p>出场检测</p>' + '<p class="span">' + outquarantineinfo[0].detectionResult + '</p>' + '<p>检测日期</p>' + '<p class="span">' + outquarantineinfo.detectionDate + '</p>' + '<p>追溯码</p>' + '<p class="span">' + data[m][arr[m]].transId + '</p>' + '</div>' + '</li>';
		...
		if (oneObj.salemarketId != '999999999' && yici == '1') {
			if (oneObj.goodsCode == '01351014' || oneObj.goodsCode == '01351014' || oneObj.goodsCode == '01342309' || oneObj.goodsCode == '01342100' || oneObj.goodsCode == '01342200') {
				if (oneObj.salermarketName.indexOf('九州') != -1) {
					htmlStr_right_add = '<li>' + '<span class="yellow_spot ani" swiper-animate-effect="bounceInDown" swiper-animate-duration="1s" swiper-animate-delay=' + '1.2' + 's></span>' + '<div class="text ani" swiper-animate-effect="bounceInRight" swiper-animate-duration="1s" swiper-animate-delay=' + '1.6' + 's>' + '<p>临沂乐超物流仓储有限公司</p>' + '<p>' + oneObj.indate.split(' ')[0] + '</p>' + '</div>' + '</li>';
					yici = 0;
				}
			}
		}
